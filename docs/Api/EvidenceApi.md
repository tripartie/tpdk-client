# Tripartie\Tpdk\EvidenceApi

All URIs are relative to https://staging-api.tripartie.com, except if the operation defines another base path.

| Method | HTTP request | Description |
| ------------- | ------------- | ------------- |
| [**apiEvidencesIdGet()**](EvidenceApi.md#apiEvidencesIdGet) | **GET** /evidences/{id} | Retrieves a Evidence resource. |


## `apiEvidencesIdGet()`

```php
apiEvidencesIdGet($id): \Tripartie\Tpdk\Model\Evidence
```
...
$apiInstance = new Tripartie\Tpdk\Api\EvidenceApi(
...
try {
    $result = $apiInstance->apiEvidencesIdGet($id);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling EvidenceApi->apiEvidencesIdGet: ', $e->getMessage(), PHP_EOL;
}
```
...
[**\Tripartie\Tpdk\Model\Evidence**](../Model/Evidence.md)
